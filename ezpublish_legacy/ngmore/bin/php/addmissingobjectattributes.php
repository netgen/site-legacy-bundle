...
$changeSiteAccessSettingClosure = function( $siteAccess )
{
    $cli = eZCLI::instance();
    ...
    {
        $cli->output( "Siteaccess $siteAccess does not exist, using default siteaccess" );
    }
};

if ( $siteAccess )
{
    $changeSiteAccessSettingClosure( $siteAccess );
}
...
$updateClassClosure = function( $classId ) use ( $cli, $script, $db, $scheduledScript )
{
...
{
    $updateClassClosure( $options['classid'] );
}
else
{
    $cli->output( 'The classid parameter was not given, will check all classes.' );
    $classes = eZContentClass::fetchAllClasses( false );
    foreach ( $classes as $class )
    {
        $cli->output( 'Checking class ' . $class['id'] . ': ' . $class['name'] );
        $updateClassClosure( $class['id'] );
    }
}
